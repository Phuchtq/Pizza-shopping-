...
public class CustomerSearchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Customer> fullList = cDao.getList();
        String searchValue = request.getParameter("searchValue").toLowerCase();
        for (Customer c : fullList) {
            String customerId = c.getCustomerId().toLowerCase();
            String customerName = c.getContactName().toLowerCase();
            if (customerId.contains(searchValue) || customerName.contains(searchValue)) {
                list.add(c);
            }
        }
        if (list.size() > 0) {
